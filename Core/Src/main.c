...
int main(void)
{
  ...
  TMC429_setup(&hspi1, GPIOC, GPIO_PIN_6, 9);
  TMC429_disableLeftSwitchStop(MOTOR_INDEX);
  TMC429_disableRightSwitches();
  TMC429_setVelocityMode(MOTOR_INDEX);
  TMC429_setLimitsInHz(MOTOR_INDEX, VELOCITY_MIN, VELOCITY_MAX, ACCELERATION_MAX);

  TMC429_disableLeftSwitchStop(1);
  TMC429_setVelocityMode(1);
  TMC429_setLimitsInHz(1, VELOCITY_MIN, VELOCITY_MAX, ACCELERATION_MAX);


  target_velocity = 0;
  delta_velocity = VELOCITY_INC;
  TMC429_setTargetVelocityInHz(MOTOR_INDEX, target_velocity);
  TMC429_setTargetVelocityInHz(1, target_velocity);
